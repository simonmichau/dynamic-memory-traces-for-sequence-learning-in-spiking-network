...
class SEMLiquid(object):
    ...
    def simulate(self, Tsim, titlestr="learning", savestrprefix="sem_liquid_train", do_plot=True):
        ...
        fig, axes = plt.subplots(nrows=9, ncols=n_neurons, figsize=(10, 18))
        n_inp_neurons = 1

        for neuron in range(n_neurons):
            # plotting_vars = {'u_trace': [], 'u_inp_trace': [], 'spk_out_trace': [], 'epsp1': [], 'epsp2': []}
            plotting_vars = {'u_trace': [], 'u_inp_trace': [], 'spk_out_trace': [], 'epsp': [], 'udyn': [], 'rdyn': [], 'dEPSP': [], 'isi': []}
            ...
            for src in range(n_neurons):
                axes[axes_idx][neuron].plot(numpy.arange(1, len(rec_weights[:, neuron, src]) + 1),
            ...
            axes[axes_idx][neuron].legend()
            axes[axes_idx][neuron].set_title('Recurrent weights')
