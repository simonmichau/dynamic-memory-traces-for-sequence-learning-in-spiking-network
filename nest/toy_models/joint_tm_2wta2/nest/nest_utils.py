...
class Recorder:
    ...
    def run_network(self, id_list: list = None, node_collection=None, readout_size: int = None,
        ...
        if self.create_plot:
            ...
            # Initialize plot
            fig, axes = plt.subplots(6, 1, sharex=not self.plot_history)  # only sync x axis if not the whole history is observed
            fig.set_figwidth(15)
            ...
            results['inp_spikes'] = inpgen.spike_generators.get('spike_times')
            ...
            for idx, neuron_gid__ in tqdm.tqdm(enumerate(node_collection), desc="Sorting neurons:", total=len(node_collection)): #enumerate(np.unique(dmm["events"]["senders"])):  # iterate over all sender neurons
                neuron_gid = neuron_gid__.global_id
                # MULTIMETER
                dmm = self.network.multimeter[idx].get()
                Vms = dmm["events"]["V_m"]
                rend = dmm["events"]["rate_fraction"]
                ts = dmm["events"]["times"]

                # SPIKERECORDER
                dSD = self.network.spikerecorder[idx].get("events")
                evs = dSD["senders"]
                ts_ = dSD["times"]

                # NOISERECORDER
                if inpgen.use_noise:
                    nr = inpgen.noiserecorder[idx].get("events")
                    evs__ = nr["senders"]
                    ts__ = nr["times"]
                ...
                src_gids = node_collection.global_id  # np.unique(dmm["events"]["senders"]).tolist()
                colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k']

                # neuron_gid = all_senders[neuron_order_idx - 1]
                ...
                if idx < 20:
                    axes[0].plot(ts, Vms, label=f'neuron: {neuron_gid}')  # plot the membrane potential of 'neuron'
                    results['Vms'][neuron_gid] = (ts, Vms)
                ...
                axes[1].plot(ts_, [idx] * len(ts_), ".", ms=5)

                results['spikes'][neuron_gid] = ts_
                ...
                if True: # plot_epsp and plot_weights
                    ...
                    for src_idx, src in enumerate(src_gids):
                        ...
                        axes[3].plot(filtered_epsps_dict['t'], filtered_epsps_dict['epsp'], color=c)  # label=f'{src} -> {neuron}'

                        if neuron_gid not in results['epsps']:
                            results['epsps'][neuron_gid] = {}
                        results['epsps'][neuron_gid][src] = (filtered_epsps_dict['t'], filtered_epsps_dict['epsp'])
                    ...
                    for src_idx, src in enumerate(src_gids):
                        ...
                        axes[4].plot(filtered_weights_dict['t'], filtered_weights_dict['w'], color=c)  # label=f'{src} -> {neuron}'

                        if neuron_gid not in results['rec_weights']:
                            results['rec_weights'][neuron_gid] = {}
                        results['rec_weights'][neuron_gid][src] = (filtered_weights_dict['t'], filtered_weights_dict['w'])

                    # INPUT WEIGHTS
                    spikegen_ids = [inpgen.spike_generators.global_id] if isinstance(inpgen.spike_generators.global_id, int) else inpgen.spike_generators.global_id
                    for src_idx, src in enumerate(spikegen_ids):
                        c = colors[src_idx % len(colors)]
                        filtered_weights_dict = {'t': [], 'w': []}
                        for t, src_, tgt_, w in self.network.weight_recorder:
                            if src_ == src and tgt_ == neuron_gid:  # and t >= t_sim_start
                                ...
                                filtered_weights_dict['w'].append(w)
                        # if len(filtered_weights):
                        axes[5].plot(filtered_weights_dict['t'], filtered_weights_dict['w'], color=c)

                        if neuron_gid not in results['inp_weights']:
                            results['inp_weights'][neuron_gid] = {}
                        results['inp_weights'][neuron_gid][src] = (filtered_weights_dict['t'], filtered_weights_dict['w'])
            ...
            #axes[3].set_ylabel("w")
            # axes[3].legend()
            axes[5].set_ylabel("Input weights")
        ...
        import pickle
        with open('../nest_data.pkl', 'wb') as f:
