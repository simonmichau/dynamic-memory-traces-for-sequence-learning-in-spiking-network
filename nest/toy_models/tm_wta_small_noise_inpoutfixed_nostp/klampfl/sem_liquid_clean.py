...
class SEMLiquid(object):
    ...
    def process_input(self, x, t=None):
        ...
        if shared_params.use_fixed_spike_times:
            self.Z = numpy.zeros(self.Z.shape).astype(int)
    ...
    def simulate(self, Tsim, titlestr="learning", savestrprefix="sem_liquid_train", do_plot=True):
        ...
        for step in range(shared_params.sim_time):
            ...
            if not self.use_inputs:
                ...
                assert shared_params.use_fixed_spike_times
                if shared_params.use_fixed_spike_times:
                    x = numpy.zeros(self.nInputs, dtype='int')
        ...
        fig, axes = plt.subplots(nrows=6, ncols=1, figsize=(10, 35), sharex=True)
        n_inp_neurons = 1
        ...
        for neuron in range(n_neurons):
            ...
            for i in range(len(self.u_trace)):
                ...
                plotting_vars['u_inp_trace'].append(self.u_inp_trace[i])  # input spikes
            ...
            for k in sorted(plotting_vars.keys()):
                ...
                if k != 'spk_out_trace' and k != 'u_inp_trace':
                    ax.plot(time_axis, v, '-')
                elif k == 'u_inp_trace':
                    inp_spikes_arr = numpy.array(v)
                    for col in range(inp_spikes_arr.shape[1]):
                        idxs = numpy.where(inp_spikes_arr[:, col] == 1)[0]
                        ax.plot(time_axis[idxs], [col] * len(time_axis[idxs]), '.')
                else:
                    idxs = numpy.where(numpy.array(v) == 1)[0]
                    ax.plot(time_axis[idxs], [neuron] * len(time_axis[idxs]), '.')
                if 'epsp' not in k:
                    title = k
                else:
            ...
            if self.use_recurrent_connections:
                ...
                axes[axes_idx].set_title('No recurrent weights')
            axes[axes_idx].set_xlim([0, shared_params.sim_time])
            axes_idx += 1

            for inp_ch in range(inp_weights.shape[2]):
                axes[axes_idx].plot(time_axis,
                                    inp_weights[:, neuron, inp_ch], c=co[neuron % len(co)])
            axes[axes_idx].set_title('Input weights')
            axes[axes_idx].set_xlim([0, shared_params.sim_time])
...
def sem_liquid_pattern1(seed=None, *p):
    strain = int(shared_params.sim_time / 1000.)  # 100  # training set length (seconds)
    # strain = 10
    stest_train = 3
    stest = 3
    params = SEMLiquidParams(task='pattern',
                             nInputs=shared_params.n_inp_channels,
                             pattern_mode='random_switching',
                             sprob=0.5,
                             nPatterns=1,
                             rin=20,  # continuous input
                             tPattern=[300e-3]*1,
                             use_priors=False,
                             ...
                             use_recurrent_connections=True,
