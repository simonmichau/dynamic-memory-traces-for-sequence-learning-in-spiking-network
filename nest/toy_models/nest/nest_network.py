...
class Network(object):
    ...
    def form_connections(self) -> None:
        ...
        print("Connected WTA circuits:")
        print(nest.GetConnections())

        # Randomize weights of each WTA circuit
        # for i in range(len(self.circuits)):  # TODO revert
...
if __name__ == '__main__':
    ...
    if task == 'regen_models':
        utils.generate_nest_code(NEURON_MODEL, SYNAPSE_MODEL, regen=True, target=NEURON_MODEL + "_with_" + SYNAPSE_MODEL)
        exit()
    else:
        utils.generate_nest_code(NEURON_MODEL, SYNAPSE_MODEL, regen=regen)
    ...
    utils.measure_network(grid, inpgen=inpgen, t_sim=shared_params.sim_time)
    print(nest.GetConnections())
