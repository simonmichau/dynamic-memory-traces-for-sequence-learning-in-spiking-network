        ...
        for i in range(len(self.circuits)):
            ...
            for j in range(len(self.circuits)):
                if i != j:
                    ...
                    conn_dict['p'] = 1 #self.lam * math.exp(-self.lam * d) # TODO: Note that this is only to make sure the toy models WTAs are connected
...
if __name__ == '__main__':
    ...
    grid = Network(grid_shape=(1, 2), k_min=2, k_max=2, n_inputs=1)
    ...
    print(grid.get_node_collections().recordables)
    print(nest.GetConnections())

    recorder = utils.Recorder(grid, save_figures=False, show_figures=True, create_plot=True)
    id_list = recorder.run_network(inpgen=inpgen, t_sim=1000, dt_rec=2, title="Simulation")
    recorder.set(create_plot=True)
    recorder.run_network(inpgen=inpgen, t_sim=1000, dt_rec=2, title="Test", train=False, order_neurons=True)
