...
class SEMLiquid(object):
    ...
    def initialize_dynamic_synapses(self):
        if self.use_dynamic_synapses:
            ...
            # self.udyn = self.U  # TODO zbarni critical for STP
            self.udyn = copy.deepcopy(self.U)  # TODO zbarni critical for STP
    ...
    def process_input(self, x, t=None):
        ...
        if self.use_dynamic_synapses:
            ...
            #    self.isi[y==1] = 0
            #else:
            # TODO @zbarni IMPORTANT TO READ critical
            # STP seems to be halfway active even on input synapses, because rdyn[] changes upon external spikes
            # therefore, we need to ensure that no external spikes (from input) is considered
            tmp_y = copy.deepcopy(y)
            tmp_y[:self.nInputs] = 0
            r = self.rdyn[tmp_y==1]; u = self.udyn[tmp_y==1]; isi = self.isi[tmp_y==1]
            D = self.D[tmp_y==1]; U = self.U[tmp_y==1]; F = self.F[tmp_y==1]
            self.rdyn[tmp_y==1] = 1 + (r - r*u - 1)*numpy.exp(-isi/(D+eps))  # TODO is this important? u_{k-1}
            self.udyn[tmp_y==1] = U + u*(1-U)*numpy.exp(-isi/(F+eps))
            # r = self.rdyn[y==1]; u = self.udyn[y==1]; isi = self.isi[y==1]
            # D = self.D[y==1]; U = self.U[y==1]; F = self.F[y==1]
            # self.rdyn[y==1] = 1 + (r - r*u - 1)*numpy.exp(-isi/(D+eps))  # TODO is this important? u_{k-1}
            # self.udyn[y==1] = U + u*(1-U)*numpy.exp(-isi/(F+eps))
            # print('exp udyn', numpy.exp(-isi/(F+eps)))
            # print('udyn', self.udyn)
            # print('exp rdyn', numpy.exp(-isi/(D+eps)))
            # print('rdyn', self.rdyn)
            # print('self.U', self.U)
            self.isi += self.dt
            self.isi[y==1] = 0
        else:
...
def sem_liquid_pattern1(seed=None, *p):
    ...
    params = SEMLiquidParams(task='pattern',
                             ...
                             use_dynamic_synapses=shared_params.use_stp_rec,  # meaning: use_stp
