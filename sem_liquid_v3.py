    ...
    def __init__(self, task, **kwds):
        ...
        if self.task == 'pattern':
            self.inpgen_seq = SequentialPatternInputGenerator(self.nInputs, self.dt, self.rin,
                                                              self.nPatterns, self.tPattern, self.pattern_sequences,
                                                              self.rNoise, self.pattern_mode, self.sprob,
                                                              self.time_warp_range)
            if self.embedded:
                self.inpgen = EmbeddedPatternInputGenerator(self.inpgen_seq, tNoiseRange=self.tNoiseRange,
                                                            prob=self.nprob)
            else:
            ...
            self.inpgen_speech = PreprocessedSpeechInputGenerator(self.nInputs, self.dt, self.rin, self.rNoise,
                                                                  scale=50, poisson=False, digits=self.digits,
                                                                  rev_digits=self.rev_digits,
                                                                  utterances=self.utterances[:7],
            ...
            inpgen1 = SequentialPatternInputGenerator(self.nInputs / 2, self.dt, self.rin,
                                                      self.nPatterns, self.tPattern[:self.nPatterns],
                                                      self.pattern_sequences[0], self.rNoise, self.pattern_mode,
                                                      self.sprob)
            inpgen2 = SequentialPatternInputGenerator(self.nInputs - self.nInputs / 2, self.dt, self.rin,
                                                      self.nPatterns, self.tPattern[:self.nPatterns],
                                                      self.pattern_sequences[0], self.rNoise, self.pattern_mode,
            ...
            self.inpgen_seq = SequentialPatternInputGenerator(self.nInputs, self.dt, self.rin,
                                                              self.nPatterns, self.tPattern[self.nPatterns:],
                                                              self.pattern_sequences[1], self.rNoise, self.pattern_mode,
                                                              self.sprob)
            if self.embedded:
                self.inpgen_pat = EmbeddedPatternInputGenerator(self.inpgen_seq, tNoiseRange=self.tNoiseRange,
                                                                prob=self.nprob)
            else:
        ...
        self.test_inpgen = copy.deepcopy(
            self.inpgen)  # deepcopy is used here because python just creates a binding between object and target when assignments are used
        if self.task == 'pattern':
...
class SEMLiquid(object):
    def __init__(self, params, **kwds):
        ...
        self.sizes = numpy.random.randint(self.size_lims[0], self.size_lims[1] + 1, self.size) # initializes an array of 'size' with values from 'size_lims'
        ...
        self.filename = 'sem_liquid_v3.py'
        ...
        self.outputdir += "_%s_%d/" % ('michau', self.seed)
        try:
            os.makedirs(self.outputdir)
        except:
            pass
        #shutil.copy(self.filename, self.outputdir + self.filename) # why is the .py file copied to /data/?
        ...
        liquidstr = "%d SEMS with %d neurons, min size: %d, max size: %d, avg size: %f" % (self.n * self.m, self.tsize,
                                                                                           numpy.min(self.sizes),
                                                                                           numpy.max(self.sizes),
                                                                                           numpy.mean(self.sizes))
        print liquidstr
        print self.sizes
    ...
    def ind2pos(self, i):  # receives an index of a matrix and calculates the indexes coordinates from that and its format https://stackoverflow.com/questions/48135736/what-is-an-intuitive-explanation-of-np-unravel-index
        if hasattr(i, '__iter__'):  # if i has the attribute '__iter__'
    ...
    def distance_probability(self, d):  #
        dmin = 1.0
        dmax = numpy.linalg.norm(numpy.array(self.size))  # calculates Frobenius norm of
        dd = 0.01
    ...
    def initialize_dynamic_synapses(self):
        if self.use_dynamic_synapses:
            ...
            if self.use_multiple_synapses:
                self.U = self.U_mean + self.U_mean / 2 * numpy.random.randn(self.tsize + self.nReadouts,
                                                                            self.nInputs + self.tsize)
                self.D = self.D_mean + self.D_mean / 2 * numpy.random.randn(self.tsize + self.nReadouts,
                                                                            self.nInputs + self.tsize)
                self.F = self.F_mean + self.F_mean / 2 * numpy.random.randn(self.tsize + self.nReadouts,
                                                                            self.nInputs + self.tsize)
    ...
    def process_input(self, x, t=None):
        ...
        if self.use_poisson_correction:
            ...
            self.poiss_corr *= ((float)(
                numpy.min(self.step, self.poisson_correction_steps))) / self.poisson_correction_steps
        ...
        if self.do_train:
            ...
            if self.use_weight_regularization:
                if self.prior == 'lognormal':
                    ...
                    B1 = numpy.minimum(B1, self.etas);
                    B1 = numpy.maximum(B1, -self.etas);
                    B2 = numpy.minimum(B2, self.etas);
                    B2 = numpy.maximum(B2, -self.etas);
    ...
    def simulate(self, Tsim, titlestr="learning", savestrprefix="sem_liquid_train", do_plot=True):
        ...
        for step in range(nsteps):
            ...
            if (step + 1) % self.dt_out == 0:
                ...
                printf('\rsimulating step %d/%d (%d%%) ETA %s...' % ((step + 1) / self.dt_out,
                                                                     nsteps / self.dt_out, ratio * 100,
                                                                     format_time((1.0 / ratio - 1) * time)))
            ...
            if step >= rec_start and step < rec_start + self.nstepsrec:
                ...
                if self.task == 'xor_pattern':
                    if hasattr(self.inpgen.inpgen,
                               'last_pattern_start') and step == self.inpgen.inpgen.last_pattern_start:
        ...
        if do_plot:
            order, times, Tord, order2, times2, Tord2 = self.get_order(pdict[spk_rec_times[-1]],
                                                                       Idict[spk_rec_times[-1]],
                                                                       tdict[spk_rec_times[-1]], rend,
                                                                       spk_rec_times[-1], self.nstepsrec)
            sub_time = 20
            sub_neurons = 1
    ...
    def apply_readouts(self, rdt_id, train_readouts, X, target):
        ...
        if self.task == 'multi':
            ...
            if self.task == 'speech':
                ...
                if len(tcl) > 0:
                    perf = [
                        float(numpy.sum(T[trs[tcl], i] == 2 * ((rdt_out[trs[tcl], i] >= 0).astype(int)) - 1)) / len(tcl)
                        for i in range(nout)]
                else:
    ...
    def test(self, Tsim, itest, start=0, plot_readout_spikes=False, train_readouts=True,
        ...
        for step in range(nsteps):
            ...
            if self.task == 'xor_pattern' and hasattr(self.test_inpgen.inpgen,
                                                      'last_pattern_start') and step == self.test_inpgen.inpgen.last_pattern_start:
                p.append(step)
            ...
            if not isppt:
                ...
                if self.task == 'multi':
                    ...
                    if self.nReadouts > 0:
                        ...
                        pylab.gca().set_xticklabels([])
                        # pylab.yticks(hlines)
                        readout_rates = numpy.mean(numpy.reshape(rzr, (nsteps / sub_time, sub_time, self.nReadouts)),
                                                   axis=1)
                        # print readout_rates.shape
                        pylab.pcolor(numpy.arange(0, nsteps + 1, sub_time), numpy.arange(self.nReadouts + 1),
                                     readout_rates.T,
                    ...
                    if self.task == 'speech':
                        ...
                        if len(self.rev_digits) == 0:
                            ...
                            pylab.yticks([-0.4, 0, 0.4],
                                         ["d%d" % (self.digits[0]), "0", "d%drev" % (self.rev_digits[0])])
...
def sem_liquid_pattern4(seed=None, *p):
    ...
    X, I, perf = liquid.test(stest, itest, titlestr=test_titlestr,
                             ...
                             train_readouts=False, do_plot=True, do_save=True)
...
def sem_liquid_pattern5(seed=None, *p):
    ...
    X, I, perf = liquid.test(stest, 2 * itest, titlestr=test_titlestr,
                             savestrprefix="sem_liquid_test%d0_inp" % (itest), plot_readout_spikes=False,
                             train_readouts=False, do_plot=True, do_save=True)
    X, I, perf = liquid.test(stest, 2 * itest + 1, titlestr=test_titlestr,
                             savestrprefix="sem_liquid_test%d1_inp" % (itest), plot_readout_spikes=False,
                             train_readouts=False, do_plot=True, do_save=True)
...
def sem_liquid_pattern8(seed=None, *p):
    ...
    X, I, perf = liquid.test(stest_train, itest, titlestr=test_titlestr,
                             ...
                             train_readouts=True, do_plot=True, do_save=True)
